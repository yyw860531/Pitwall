...
No tools: receives pre-computed corner analyses, braking results, and balance
results from the orchestrator. Uses run_agent for consistency but never calls tools.
"""

import json
import logging

from pitwall.agents._base import run_agent, load_prompt, TOOLS_COACHING_WRITER
...
def write(
	...
	try:
		result = run_agent(
			_SYSTEM,
			user,
			allowed_tools=TOOLS_COACHING_WRITER,
			max_tokens=4096,
		)
